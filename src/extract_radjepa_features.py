"""
Extract RadJEPA mean-pooled patch features from MIMIC-CXR and save each as a
per-image .npy file co-located with the source image:

    mimic-cxr-jpg/files/p10/p10000032/s50414267/radjepa_<dicom_id>.npy  (768,) float32
...
TRANSFORM = transforms.Compose([
    ...
    transforms.ToTensor(),
    transforms.Normalize(mean=MIMICCXR_MEAN, std=MIMICCXR_STD),
])
...
def _feat_path(root: str, row) -> str:
    return os.path.join(_study_dir(root, row), f"{FEAT_PREFIX}{row['dicom_id']}.npy")
...
def collate_fn(batch):
    ...
    dids  = [b[1] for b in batch]
    sids  = [b[2] for b in batch]
    stids = [b[3] for b in batch]
    return imgs, dids, sids, stids
...
def extract_split(model, root, split, batch_size, num_workers, rank, world_size):
    ...
    with torch.no_grad():
        for imgs, dids, sids, stids in tqdm(loader, desc=f"  GPU{rank}/{split}", unit="batch"):
            feats = model(pixel_values=imgs.cuda())   # (B, 768) — mean-pooled directly
            feats = feats.cpu().float().numpy()

            for i, (did, sid, stid) in enumerate(zip(dids, sids, stids)):
                out_path = os.path.join(
                    root, "files", f"p{sid[:2]}", f"p{sid}", f"s{stid}",
                    f"{FEAT_PREFIX}{did}.npy"
                )
...
def worker(rank: int, world_size: int, args):
    ...
    print(f"[GPU {rank}] Loading RadJEPA...")
    model = AutoModel.from_pretrained(
        "AIDElab-IITBombay/RadJEPA", trust_remote_code=True
    ).cuda().eval()
